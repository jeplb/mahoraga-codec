...
outputs:
    ...
    outputs/shannon_fraction_table.tex   wide LaTeX fragment
...
OUT_TEX = REPO_ROOT / "outputs" / "shannon_fraction_table.tex"
...
CODEC_LABELS = {"mahoraga": "Mahoraga", "dna_aeon": "DNA-Aeon", "mgcplus": "MGC+"}
...
def write_latex_table(df: pd.DataFrame, out_path: Path) -> None:
    """wide LaTeX fragment: (channel, r, shannon bound, codec columns)."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    channels = ["hifi", "lofi"]
    r_values = sorted(df["r"].unique())

    codec_hdr = " & ".join(
        f"\\textbf{{{CODEC_LABELS[c]}}}" if c == "mahoraga" else CODEC_LABELS[c]
        for c in CODECS
    )
    lines = [
        ...
        "\\toprule",
        "Channel & $r$ & Shannon [EB\\,g$^{-1}$] & " + codec_hdr + " \\\\",
        "\\midrule",
    ]

    for channel in channels:
        for r in r_values:
            sub = df[(df["channel"] == channel) & (df["r"] == r)]
            if sub.empty:
                continue
            bound = sub["shannon_bound_ebpg"].iloc[0]
            cells = []
            for codec in CODECS:
                row = sub[sub["codec"] == codec]
                if row.empty:
                    cells.append("--")
                    continue
                frac = row["shannon_fraction_pct"].iloc[0]
                cell = _fmt_pct(frac)
                if codec == "mahoraga" and cell != "--":
                    cell = f"\\textbf{{{cell}}}"
                cells.append(cell)
            lines.append(
                f"{channel.upper()} & {r:g} & {bound:.1f} & "
                + " & ".join(cells) + " \\\\"
            )
